...
const renderWithRouterr = (component) => {
...
test('link about should render about page', () => {
  const { getByText, history } = renderWithRouterr(<App />);
...
test('about page should have heading About pokedex', () => {
  const { getByRole } = renderWithRouterr(<About />);
...
test('about page should have two paragraphs', () => {
  const { getByText } = renderWithRouterr(<About />);
